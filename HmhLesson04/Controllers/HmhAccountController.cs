...
namespace HmhLesson04.Controllers
{
    public class HmhAccountController : Controller
    {
        ...
        public IActionResult HmhProfile(int? id)
        {
            HmhAccount hmhAccount = new HmhAccount
            {
                Id = 5,
                Name = "Đặng Quốc Bảo",
                Email = "bao.dang@example.com",
            ...
            if (id != null)
            {
                hmhAccount = hmhAccounts.FirstOrDefault(x => x.Id == id);
            }
            ViewBag.HmhAccount = hmhAccount;
            return View(); 
        }
    }
}
